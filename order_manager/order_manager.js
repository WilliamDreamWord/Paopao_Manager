...
function show() {
    ...
    console.log("查询条件为：" + "order_type=" + order_type + "order_area=" + order_area + "order_status=" + order_status + "order_date=" + date);
    ...
    setTimeout(function () {
        ...
        if (order_number == "") {
            ...
            console.log("开始执行了默认查询")

            // 2. 设置回调监听
            select_date_status_Http.onreadystatechange = function () {

                if (4 == select_date_status_Http.readyState && select_date_status_Http.status == 200) {
            ...
            order_detail_Http.onreadystatechange = function () {
                ...
                if (4 == order_detail_Http.readyState && order_detail_Http.status == 200) {

                    console.log(JSON.parse(order_detail_Http.responseText));

                    order_detail_content = JSON.parse(order_detail_Http.responseText);

                    if (JSON.stringify(order_detail_content.data.shipping) == "null") {
                        console.log("该条订单没有收货地址")
                        alert("该条订单没有收货地址")
                        // break;
                    } else {
                        if (order_detail_content.code == 1) {
                            alert("身份信息已过期，请重新登录")
                        } else {
                ...
                } else if (4 == order_detail_Http.readyState && order_detail_Http.status == 500) {
                    alert("暂无该id订单")
                }
...
window.onload = function () {
    ...
    //设置当前月份为默认月份
    months.options[nowMonth - 1].selected = true;
    ...
    //设置当前天为默认天
    days.options[nowDate - 1].selected = true;
}
